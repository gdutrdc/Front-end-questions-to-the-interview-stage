...
----------------------
    ...
    注意：以上讨论均只同一域名（参考同源策略）。以IE7为例，如果资源来自2个不同的域名，则可有12个并发数量。这也就是为什么要将静态资源放到CDN
...
注：经过测试`IE6,7,8,9`均不支持严格模式。ES6默认开启了严格模式。

缺点：
现在网站的`JS` 都会进行压缩，一些文件用了严格模式，而另一些没有。这时这些本来是严格模式的文件，被 `merge` 后，这个串就到了文件的中间，不仅没有指示严格模式，反而在压缩后浪费了字节。（解决方法：不要申明全局的严格模式、应该只在函数体里申明）
...
-------
    ...
    304  Not Modified  自从上次请求后，请求的网页未修改过。（对于缓存来说非常重要）
